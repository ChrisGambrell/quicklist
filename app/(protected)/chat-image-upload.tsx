...
import { createListingImage } from '@/actions/listing'
...
export default function ChatImageUpload() {
	const [state, action] = useFormState(createListingImage, null)
	...
	return (
		<div className='grid gap-4 max-w-md mx-auto border rounded-xl p-6 shadow-sm'>
			...
			<form action={action} className='grid gap-4'>
				<input type='hidden' name='testing_images' value={images.map((image) => image.name)} />
